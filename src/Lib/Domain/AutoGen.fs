...
module CharacterSheet =
    ...
    let stats_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.stats)
            (fun (value: Stats) (data: CharacterSheet) -> { data with stats = value })

    let unmodifiedStats_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.unmodifiedStats)
            (fun (value: Stats) (data: CharacterSheet) -> { data with unmodifiedStats = value })

    let name_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.name)
            (fun (value: string) (data: CharacterSheet) -> { data with name = value })

    let sex_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.sex)
            (fun (value: Sex) (data: CharacterSheet) -> { data with sex = value })

    let race_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.race)
            (fun (value: Race) (data: CharacterSheet) -> { data with race = value })

    let xp_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.xp)
            (fun (value: int) (data: CharacterSheet) -> { data with xp = value })

    let allocatedLevels_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.allocatedLevels)
            (fun (value: Class list) (data: CharacterSheet) -> { data with allocatedLevels = value })

    let subclasses_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.subclasses)
            (fun (value: Map<Class, Subclass>) (data: CharacterSheet) -> { data with subclasses = value })

    let classAbilities_ =
        Optics.lens
            (fun (data: CharacterSheet) -> data.classAbilities)
            (fun (value: ClassAbility list) (data: CharacterSheet) -> { data with classAbilities = value })

module StatBlock =
    open Domain.Model
    let stats_ =
        Optics.lens
            (fun (data: StatBlock) -> data.stats)
            (fun (value: Stats) (data: StatBlock) -> { data with stats = value })
...
module CharSheet =
    ...
    let statBlock_ =
        Optics.lens
            (fun (data: CharSheet) -> data.statBlock)
            (fun (value: StatBlock) (data: CharSheet) -> { data with statBlock = value })
    ...
    let yearOfBirth_ =
        Optics.lens
            (fun (data: CharSheet) -> data.yearOfBirth)
            (fun (value: int) (data: CharSheet) -> { data with yearOfBirth = value })

    let sex_ =
        Optics.lens
            (fun (data: CharSheet) -> data.sex)
            (fun (value: Sex) (data: CharSheet) -> { data with sex = value })

module Creature =
    open Domain.Model
    let name_ =
        Optics.lens
            (fun (data: Creature) -> data.name)
            (fun (value: string) (data: Creature) -> { data with name = value })

    let stats_ =
        Optics.lens
            (fun (data: Creature) -> data.stats)
            (fun (value: StatSource) (data: Creature) -> { data with stats = value })

module DraftSheet =
    open Domain.Model.Draft
    let unmodifiedStats_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.unmodifiedStats)
            (fun (value: Stats) (data: DraftSheet) -> { data with unmodifiedStats = value })

    let explicitName_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.explicitName)
            (fun (value: string option) (data: DraftSheet) -> { data with explicitName = value })

    let autoName_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.autoName)
            (fun (value: string) (data: DraftSheet) -> { data with autoName = value })

    let sex_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.sex)
            (fun (value: Setting<Sex>) (data: DraftSheet) -> { data with sex = value })

    let race_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.race)
            (fun (value: Setting<Race>) (data: DraftSheet) -> { data with race = value })

    let xp_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.xp)
            (fun (value: int) (data: DraftSheet) -> { data with xp = value })

    let allocatedLevels_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.allocatedLevels)
            (fun (value: Class list) (data: DraftSheet) -> { data with allocatedLevels = value })

    let subclasses_ =
        Optics.lens
            (fun (data: DraftSheet) -> data.subclasses)
            (fun (value: Map<Class, Subclass>) (data: DraftSheet) -> { data with subclasses = value })
...
module State =
    ...
    let ids_ =
        Optics.lens
            (fun (data: State) -> data.ids)
            (fun (value: IdGenerator) (data: State) -> { data with ids = value })

    let properties_ =
        Optics.lens
            (fun (data: State) -> data.properties)
            (fun (value: Map<string, Property>) (data: State) -> { data with properties = value })

    let eventDefinitions_ =
        Optics.lens
            (fun (data: State) -> data.eventDefinitions)
            (fun (value: Map<string, EventDefinition>) (data: State) -> { data with eventDefinitions = value })

    let data_ =
        Optics.lens
            (fun (data: State) -> data.data)
            (fun (value: Map<DataKey, obj>) (data: State) -> { data with data = value })

    let settled_ =
        Optics.lens
            (fun (data: State) -> data.settled)
            (fun (value: Map<RowKey, string>) (data: State) -> { data with settled = value })

    let outstandingQueries_ =
        Optics.lens
            (fun (data: State) -> data.outstandingQueries)
            (fun (value: Map<DataKey, Logic<unit> list>) (data: State) -> { data with outstandingQueries = value })

    let workQueue_ =
        Optics.lens
            (fun (data: State) -> data.workQueue)
            (fun (value: Logic<unit> Queue.d) (data: State) -> { data with workQueue = value })

    let log_ =
        Optics.lens
            (fun (data: State) -> data.log)
            (fun (value: RowKey Queue.d) (data: State) -> { data with log = value })
