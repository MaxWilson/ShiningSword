...
type Property<'t>(name, runtimeType) =
    ...
    abstract Set: Id *'t -> DeltaState -> DeltaState
    abstract SetM: Id * 't -> StateChange<DeltaState, unit>
and [<AbstractClass>]
    Expression<'t>() =
...
and Affordance = {
    name: Name
    action: Id -> DeltaState -> DeltaState
    }
...
and DeltaState = Delta.DeltaDrivenState<Ribbit, RibbitMsg>

type FightResult = Victory | Defeat | Ongoing
type RoundResult = { outcome: FightResult; msgs: string list; ribbit: DeltaState }
...
type NumberProperty(name, defaultValue: _ option) =
    ...
    override this.Set(rowId, value) (state: DeltaState) =
...
type FlagsProperty<'t>(name, defaultValue: string Set) =
    ...
    member this.SetFlag(rowId, targetFlag:'t, value) (state: DeltaState) =
    ...
    member this.SetAllM(rowId, value) (state: DeltaState) = (), this.SetAll(rowId, value) state
    member this.SetFlagM(rowId, targetFlag, value) (state: DeltaState) = (), this.SetFlag(rowId, targetFlag, value) state
