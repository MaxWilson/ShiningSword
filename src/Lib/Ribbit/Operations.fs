...
let nextId(): StateChange<DeltaState, Id> = stateChange {
...
let findTarget ids id = stateChange {
    ...
    let! isAlive = Delta.getM (fun ribbit -> (currentTarget > 0) && (hpP.Get id ribbit > damageTakenP.Get currentTarget ribbit))
    if isAlive then
