...
let processMagicSpells txt =
    let mutable out = []
    let rec recur prior = function
        | line::rest ->
            match System.Text.RegularExpressions.Regex.Match(line, @"^\d+(.*)") with
            | m when m.Success ->
                let prior = defaultArg prior ""
                out <- ($"{m.Groups[1].Value |> fun s -> s.Trim()} {prior}") :: out
                recur None rest
            | _ ->
...
let magicSpells = magic.Split("\r\n") |> List.ofArray |> List.map (fun s -> s.Trim()) |> processMagicSpells

let dfSpells = dfSpells |> eachLine DF.parse |> List.map (function Ok (spell, _) -> spell.name, spell | _ -> notImpl()) |> Map.ofList
#r "nuget: TextCopy"
let out = System.Text.StringBuilder()
for line in magicSpells do
    let equivalent =
        let matches = dfSpells.Keys |> Seq.map (fun spell -> if line.StartsWith spell then spell, spell.Length else "", 0)
        match matches |> Seq.maxBy snd with
        | name, l when l > 0 -> Some name
        | _ -> None
    match equivalent with
    | Some spellName ->
        let spell = dfSpells[spellName]
        out.AppendLine $"""{spell.name} {spell.prereqs |> List.map (fun p -> p.items |> String.join ", " |> sprintf "%A: [%s]" p.heading) |> String.join " or "}
    WAS {line}"""
            |> ignore
    | _ ->
        ()//out.AppendLine $"{line} is not in DF"
out.ToString() |> TextCopy.ClipboardService.SetText
