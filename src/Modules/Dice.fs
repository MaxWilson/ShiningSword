...
let render combineLines (r:Roll) =
...
let toLogChunk (explanation: Explanation) : Log.Chunk =
...
module Parse =
    ...
    (|SimpleRoll|_|) <-
        ...
        | _ -> None
    let mutable (|Roll|_|) = uninitialized<Roll>
    ...
    let (|Branch|_|) = pack <| function
        | TestVariable((tv,condition), Roll(r1, Str ":"(Roll(r2, rest)))) -> Some(Roll.Request.Branch(tv, [condition, r1; Else, r2]), rest)
        | TestVariable((tv,condition), Roll(r, rest)) -> Some(Roll.Request.Branch(tv, [condition, r]), rest)
        | TestVariable((tv,condition), rest) -> Some(Roll.Request.Branch(tv, [condition, StaticValue 1]), rest)
        | _ -> None
