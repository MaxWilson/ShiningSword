...
let pack (rule: ParseRule<'t>) : ParseRule<'t> =
    ...
    let eval (input: ParseInput) =
        ...
        let key = (pos, id)
        ctx.active.Value <- ctx.active.Value.Remove key // mark visited
        ...
        | None -> // nothing settled yet--we have to grow a match or failure
            ...
            let active = ctx.active.Value
            ctx.active.Value <- active.Add key
            ctx.settled.Value <- settled.Add(key, Fail) // initialize intermediate set to failure to prevent infinite left-recursion
            ...
            ctx.active.Value <- ctx.active.Value.Remove key // Clean up after ourselves, though it shouldn't be necessary
            let grow seed settled =
                let rec grow seed settled =
                    // update the intermediate cache before re-evaluating
                    ctx.settled.Value <- settled
                ...
                match grow seed settled with
                    | Some(v, endpos) ->
                        ctx.settled.Value <- (settled |> Map.add key (Success (v, endpos))) // remember the largest success
                        Some(unbox v, (ctx, endpos))
                    | None ->
                        ctx.settled.Value <- (settled |> Map.add key Fail) // remember the failure
                        None
            ...
            | None ->
                if hadLeftRecursion then
                    ...
                    ctx.settled.Value <- ctx.settled.Value |> Map.add key Fail // remember the failure
                    None
            | Some(v, ((ctx, outpos) as output)) ->
                if hadLeftRecursion then
                    ...
                    ctx.settled.Value <- ctx.settled.Value |> Map.add key (Success (box v, outpos)) // remember the success
                    Some(v, output)
...
#nowarn "40" // we're not doing anything weird here like calling passed-in functions from inside a constructor, so warning 40 doesn't apply
let rec (|Words|_|) = pack <| function
